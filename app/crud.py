...
from models import Item, Endereco
from schemas import SchemaItemCreate, SchemaEnderecoCreate
...
def get_endereco(db: Session, item_id: int):
    return db.query(Endereco).filter(Endereco.id == item_id).first

#Criar itens em formato de dicionario

def create_endereco(db: Session, item: SchemaEnderecoCreate):
    db_item = Endereco(**item.dict())
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
